...
- Distribution: The CLI is downloaded from the official GitHub release during setup or repair and verified against the checksum published with that release. It is not embedded in the public BD-AUTO setup executable.
...
BD-AUTO's public repository and setup executable do not contain plugin or theme source files. `payload/addons.manifest.json` contains pinned HTTPS locations. During setup, each user's computer downloads the selected files directly from the authors' upstream repositories and validates the declared add-on metadata before installation.
...
| --- | --- | --- | --- | --- | --- |
| `0BDFDB.plugin.js` | BDFDB | DevilBro | 4.5.3 | [mwittrien/BetterDiscordAddons](https://github.com/mwittrien/BetterDiscordAddons) | [GPL-2.0](https://github.com/mwittrien/BetterDiscordAddons/blob/master/LICENSE) |
...
| `CallTimeCounter.plugin.js` | CallTimeCounter | QWERT, KingGamingYT; repository also credits Rasync as original creator | 1.0.2 | [KingGamingYT/CallTimeCounter](https://github.com/KingGamingYT/CallTimeCounter) | No license detected |
...
As of June 9, 2026, GitHub did not detect a license file for these pinned upstream repositories:
