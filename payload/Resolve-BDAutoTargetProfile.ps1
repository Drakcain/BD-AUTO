...
function Resolve-BDAutoTargetProfile {
  ...
  $discordProcesses = @(Get-CimInstance Win32_Process -Filter "Name='Discord.exe'" -ErrorAction SilentlyContinue)
  ...
  $discordProfiles = @(Get-CimInstance Win32_UserProfile -ErrorAction SilentlyContinue |
    Where-Object {
      $_.LocalPath -and
      -not $_.Special -and
  ...
  $currentProfile = Get-CimInstance Win32_UserProfile -ErrorAction SilentlyContinue |
