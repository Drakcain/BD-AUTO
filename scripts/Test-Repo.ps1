...
Test-Condition -Name 'Build bdcli fallback behavior' -Passed (
  ...
  $buildScript -match 'Using bundled BetterDiscord CLI'
) -Detail 'build can fall back to bundled/local bdcli when live API is unavailable'
...
Test-Condition -Name 'Discord channel roots from LOCALAPPDATA' -Passed ($profileResolver -match 'DiscordPTB' -and $profileResolver -match 'DiscordCanary' -and $profileResolver -match 'Get-BDAutoDiscordChannelRoots') -Detail 'stable/PTB/Canary roots derived from LocalAppData'
...
Test-Condition -Name 'Scheduled task auto-update workflow' -Passed ($taskScript -match '-WaitForDiscord' -and $taskScript -match '-RepairAfterDiscordUpdate' -and $taskScript -match '-ReopenDiscord') -Detail 'sign-in/wake task can apply staged Discord updates before BD repair'
...
Test-Condition -Name 'Discord update check command' -Passed ($watchdogScript -match '\[switch\]\$CheckDiscordUpdateState' -and $watchdogScript -match 'Get-DiscordWorkflowState' -and $watchdogScript -match 'Write-DiscordWorkflowState') -Detail 'explicit Discord update state command present'
Test-Condition -Name 'Discord update monitor command' -Passed ($watchdogScript -match '\[switch\]\$MonitorDiscordUpdate' -and $watchdogScript -match 'Wait-DiscordUpdateTransition' -and $watchdogScript -match 'MonitorTimeoutMinutes') -Detail 'bounded Discord update monitor present'
Test-Condition -Name 'Repair-after-update command' -Passed ($watchdogScript -match '\[switch\]\$RepairAfterDiscordUpdate' -and $watchdogScript -match 'Pending Discord update detected' -and $watchdogScript -match 'local Discord Update.exe flow') -Detail 'explicit post-update repair path present'
...
Test-Condition -Name 'No update-all behavior' -Passed ($watchdogScript -notmatch 'winget upgrade --all|upgrade --all|choco upgrade all|scoop update \*') -Detail 'Discord-only scope preserved'
